...
- **Contract final**: se încarcă din modalul de documente sau din „Editează". `lib/contract-final-import.ts` verifică să fie al dosarului și completează zile, valoare (cu/fără TVA), clasă etc. Un fișier greșit e respins fără să încarce nimic. Acceptă și **scanări/poze**: PDF fără text sau imagine → OCR Tesseract (`textContract`); regex-urile din `contract-final.ts` tolerează greșeli OCR („și”, „;”, „R8H”, „14,09.2026”). Testat doar pe un contract generat, nu pe o scanare reală.
