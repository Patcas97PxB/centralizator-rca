...
- **Contract final**: se încarcă din modalul de documente sau din „Editează". `lib/contract-final-import.ts` verifică să fie al dosarului și completează zile, valoare (cu/fără TVA), clasă etc. Un fișier greșit e respins fără să încarce nimic. Acceptă și **scanări/poze**: PDF fără text sau imagine → OCR Tesseract (`textContract`); regex-urile din `contract-final.ts` tolerează greșeli OCR („și”, „;”, „R8H”, „14,09.2026”). La scan: nr. auto/contract/dosar se compară tolerant (≤2 caractere diferite, `aproapeLaFel`), nr. auto păgubit care nu arată a nr. RO e ignorat, rândul de cost se ia doar de pe linia lui (sume fără virgulă → ultimele 2 cifre = zecimale; „Total: X EUR” are prioritate). Nr. dosar poate conține cratime; la **Hellas** formatul canonic e „HDR 121926” — sufixul „-4698” de pe contract se taie (ca în `shared/extractie.js`), și la comparație (`normDosar`). Nr. rezervare se compară doar pe cifre („RBH/29770” = „RBH 29770” = „29770”). Toleranța OCR acceptă doar confuzii reale (8↔B↔S, 0↔O, 1↔I…), nu orice cifră. Testat în browser pe scanul real CONTRACT_BH88PMP (Hellas, RBH/29770, 4 zile, 128,00/154,88 EUR) — ~8 s.
...
- **Modificare PDF** (cardul din dreapta): editor în browser, fără API/AI/server. `lib/pdf-editor.ts` + `ModificarPdfModal.tsx`. MuPDF.js (WASM, licență AGPL, încărcat lazy) citește cuvintele cu poziții și **șterge definitiv** textul ales; pdf-lib scrie noul text în același loc (font, mărime și culoare din original; Arimo ca rezervă pentru ș/ț/ă). **Nuanța se potrivește vizual** (`tonZona`): se măsoară cât de închis arată originalul randat și, dacă textul nou iese mai apăsat (transparență pierdută, font mai gros), culoarea se deschide până la același ton — doar deschide, nu intunecă. După ștergere verifică să nu se fi schimbat altceva, altfel refuză. Originalul nu se modifică; ieșirea e o copie descărcată + jurnal .txt. Doar PDF-uri cu text (nu scanate). Testat pe „PV PREDARE-PRELUARE + CONTRACT" (Helvetica 7pt). **Separă pagini** (în același modal, la PDF-uri cu 2+ pagini): bifezi numerele paginilor → „Salvează … ca PDF nou” descarcă un PDF doar cu ele (`extragePagini`, pdf-lib; pleacă de la versiunea curentă, cu modificări; merge și pe scanate).
